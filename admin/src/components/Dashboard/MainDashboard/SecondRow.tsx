...
import { tableValuesLocaleConvert } from './FirstRow';
...
const SecondRow: FC = (() => {
  const { classes, theme } = secondStyle({});
  const { i18n, t } = useTranslation('dashboard')
  const rtlActive = i18n.language == 'fa';
  const objecData = useMemo(() => {
    return {};
  }, []);
  const dataTable = (mapData: MapDataType[]) => {
    let result = [];
    for (let index = 0; index < mapData.length; index++) {
      ...
      result.push([
        <img
          ...
          key={'flag'}
        />,
        element[`country_${i18n.language}` as keyof typeof element],
      ...
      <Grid item xs={12}>
        <div className={classes.card}>
          <div className={classes.cardHeader}>
            <div color='success' className={classes.cardIconsuccess}>
              <LanguageIcon className={classes.svg} />
            </div>
            {/* @ts-ignore */}
                      ...
                      dataTable(mapData).map((_data: any, index: number) => {
                        return (
                          <TableRow key={index} className={classes.tableRowBody} >
                            {
                              _data.map((a: any, i: number) => {
                                return (
                                  <TableCell
                                    key={i}
                                    align={rtlActive ? 'right' : 'left'}>
                                    {a}
                                  </TableCell>
                                )
                              })
